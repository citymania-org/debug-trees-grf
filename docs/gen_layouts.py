...
al = [l.split() for l in s.strip().split('\n') if not l.strip().startswith('/*')]
# bl = [[(int(y[:-1], 16) - 0x628) // 7 * 7 + 0x628 for y in (x[2], x[6], x[10], x[14])] for x in al]

def parse_list(l):
    r = [[(int(y[:-1], 16) - 0x628) // 7 for y in (x[2], x[6], x[10], x[14])] for x in l]
    s0 = 1 - min(min(x) for x in r)
    return [[y + s0 for y in x] for x in r]

def make_gv(l, filename):
    with open(filename, 'w') as f:
        f.write('strict digraph G {\n')
        for i, (a, b, c, d) in enumerate(l):
            f.write(f'    {b} -> {a}; {c} -> {a}; {d} -> {a};\n')
        f.write('}\n')

def make_files(l, name):
    make_gv(l, name + '.gv')
    os.system(f'dot -T png {name}.gv -o {name}.png')

# make_files(parse_list(al[:20 * 4]), 'docs/temperate_layouts')
make_files(parse_list(al[:48]), 'temperate_layouts')
make_files(parse_list(al[48:80]), 'arctic_layouts')
make_files(parse_list(al[80:128]), 'tropic_layouts')
make_files(parse_list(al[128:164]), 'toyland_layouts')
